...
export class NeonSignMenu extends Phaser.GameObjects.Container {
   ...
   private letterPositions: { x: number; y: number }[] = [
    ...
    private buildSign() {
      ...
      for(let idxLetter:number = 0; idxLetter < 13; idxLetter++) {
         ...
         const letterSprite = this.scene.add.sprite(
               xLetter, 
               yLetter, 
               'neon-letters-menu-atlas', idxLetter
            );
    ...
    private triggerBrokenEffect() {
        ...
        const candidates = [this.letters[2], this.letters[9]];
    ...
    public stopEffects() {
        if (this.humSound) {
            ...
            // Option A: Hard cut
            this.humSound.stop();
            this.humSound.destroy(); // Clean up memory
            
            // Option B: Quick fade out (more polished)
            /*
            this.scene.tweens.add({
                targets: this.humSound,
                volume: 0,
                duration: 500,
                onComplete: () => this.humSound?.stop()
            });
            */
        }
    }
